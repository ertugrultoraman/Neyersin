...
export default async function SiparislerimSayfasi({
  ...
  if (!oturum) redirect("/hesap/giris?donus=/siparislerim");
  ...
  return (
    <PanelKabuk
      ...
      baglantilar={[
        { href: oturum.rol === "musteri" ? "/hesabim" : "/panel", etiket: "Profilime dön" },
      ]}
    >
      {/* Sekmeler — yalnızca birden fazla liste varsa anlamlı */}
      {!tekSekme && (
        <div role="group" aria-label="Sipariş türü" className="mt-8 flex flex-wrap gap-2">
          {sekmeler.map((s) => {
            ...
            return (
              <Link
                key={s.id}
                href={`/siparislerim?${hedef.toString()}`}
      ...
      <div className={tekSekme ? "mt-8" : "mt-5"}>
